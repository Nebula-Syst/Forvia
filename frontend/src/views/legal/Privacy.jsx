...
export default function Privacy() {
  ...
  return <div className="narrow">
    ...
    <p className="muted small" style={{ lineHeight: 1.5 }}>
      Un registro interno de actividad (inicios de sesión, acciones de administración) para poder detectar problemas de seguridad. Tu dirección IP <strong>no</strong> se guarda en ese registro por defecto en esta instancia. Si envías un reporte de error desde Ajustes, guardamos el mensaje que escribes y, si has iniciado sesión, tu nombre y correo para poder responderte. También datos de uso de la app a través de Google Analytics — ver el punto 4 y la <a onClick={e => { e.preventDefault(); nav('/legal/cookies') }} href="/legal/cookies">política de cookies</a> para el detalle completo.
    </p>
    ...
    <p className="muted small" style={{ lineHeight: 1.5 }}>
      También usamos <strong>Google Analytics</strong> para entender cómo se usa la app (qué pantallas se visitan, con qué frecuencia) y así poder mejorarla — solo una vez que has creado una cuenta e iniciado sesión, nunca en la pantalla de acceso. Esto envía datos de uso a Google, que los trata como encargado según sus propias condiciones; puedes leer las suyas en <a href="https://policies.google.com/privacy" target="_blank" rel="noopener">policies.google.com/privacy</a>. Detalle completo de qué guarda y por cuánto tiempo: <a onClick={e => { e.preventDefault(); nav('/legal/cookies') }} href="/legal/cookies">política de cookies</a>.
    </p>
    <p className="muted small" style={{ lineHeight: 1.5 }}>
      <strong>No vendemos tus datos ni los compartimos con nadie con fines publicitarios.</strong> Google Analytics es la única herramienta de terceros que recibe datos de uso, y solo para medir y mejorar el servicio — no hay anuncios ni redes de publicidad en Forvia.
    </p>

    <h4 className="sec">5. Dónde vive tu información</h4>
    <p className="muted small" style={{ lineHeight: 1.5 }}>
      Forvia se aloja en infraestructura propia de Nebula Systems (autoalojado), no en un proveedor de nube de terceros que también tenga acceso a los datos. Si el envío de correo de verificación está activo, el mensaje pasa por el servidor SMTP que Nebula Systems tenga configurado en ese momento. La única excepción es Google Analytics (punto 4): los datos de uso que genera sí salen hacia los servidores de Google, que pueden estar fuera de la Unión Europea.
    </p>
    ...
    <p className="muted small" style={{ lineHeight: 1.5 }}>
      Al ser una cuenta obligatoria para usar Forvia, no hay un interruptor dentro de la app para desactivar solo Google Analytics manteniendo la cuenta activa. Si prefieres que tu navegador no lo cargue, puedes bloquearlo tú mismo con una extensión de bloqueo de rastreadores o el complemento de inhabilitación que el propio Google ofrece.
    </p>
    ...
    <p className="muted small" style={{ lineHeight: 1.5 }}>
      Forvia usa una cookie propia, estrictamente necesaria para mantener tu sesión iniciada, y las cookies de Google Analytics (punto 4) una vez has iniciado sesión — ninguna de publicidad ni rastreo de terceros ajeno a eso. Detalle completo, incluido qué guardamos en el almacenamiento local de tu navegador (<code>localStorage</code>) y cómo borrarlo: <a onClick={e => { e.preventDefault(); nav('/legal/cookies') }} href="/legal/cookies">política de cookies</a>.
    </p>
