...
Forvia is Nebula Systems' update of openGym by Duarte Santos: two small containers (a web
server and an API) plus a folder of your data. This guide takes you from "just cloned it" to
"using it from my phone over the internet".
...
Forvia signs you in with **passkeys** (WebAuthn). Browsers enforce two rules:
...
passkey prompt won't appear. To use Forvia from your phone you need a real HTTPS hostname.
...
Put Forvia behind something that terminates TLS for a hostname you control, then point it at
...
Forvia only needs the browser to reach it over `https://gym.example.com`.
...
The defaults assume Forvia is the only thing here: a service called `api` on port 3000, and nginx
...
```bash
tar czf forvia-backup-$(date +%F).tar.gz data/
```
...
Forvia can push two kinds of alert to your phone/desktop, even when the app isn't open:
...
you about your pushes. Forvia sends your `ORIGIN` by default; set `VAPID_SUBJECT=mailto:you@example.com`
