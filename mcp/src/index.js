#!/usr/bin/env node
/* Forvia MCP server — stdio transport. The LLM client (Claude Desktop, Cursor, …) spawns
...
// Fail fast on bad config so a misnamed FORVIA_DATA doesn't silently answer every call with
...
try {
  init()
  const u = getUser()
  console.error(`[forvia-mcp] serving profile ${u.name} (${u.id})`)
} catch (e) {
  console.error(`[forvia-mcp] ${e.message}`)
