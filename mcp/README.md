...
application (Claude Desktop, Cursor, Cline, Continue, etc.) read your Forvia profile —
...
session secrets — it can only read the same `state-<uid>.json` files the Forvia api already
writes.
...
FORVIA_UID=<your-uid> FORVIA_DATA=/path/to/Forvia/data node src/index.js
```
...
```jsonc
{
  "mcpServers": {
    "forvia": {
      "command": "node",
      "args": ["/absolute/path/to/Forvia/mcp/src/index.js"],
      "env": {
        "FORVIA_DATA": "/absolute/path/to/Forvia/data",
        "FORVIA_UID": "<your-uid>"   // optional — auto-detected if you have one profile
      }
    }
  }
}
...
Restart the client; you should see the Forvia tools appear with "serving profile \<name\>" on
the server's stderr.
...
AGPL-3.0-or-later, same as Forvia.
